...
def cygwin_pidof(proc_name):
    ...
    pipe = subprocess.Popen(
        "ps -ea | grep " + proc_name, shell=True, stdout=subprocess.PIPE
    )
...
def wsl2_host_ip():
    ...
    pipe = subprocess.Popen(
        "ip route show default | awk '{print $3}'", shell=True, stdout=subprocess.PIPE
    )
...
def kill_tasks_psutil(victims):
    ...
    for proc in psutil.process_iter():
        ...
        if pdict["environ"] is not None:
            if (
                pdict["environ"].get("SIM_VEHICLE_SESSION")
    ...
    if cmd_opts.coverage:
        ...
        for proc in psutil.process_iter(["pid", "name", "environ"]):
            if proc.name() not in [
                ...
                continue
            if os.environ["SIM_VEHICLE_SESSION"] not in proc.environ().get(
                "SIM_VEHICLE_SESSION"
            ):
...
def find_location_by_name(locname):
    ...
    locations_userpath = os.environ.get(
        "ARDUPILOT_LOCATIONS", get_user_locations_path()
    )
...
def run_cmd_blocking(what, cmd, quiet=False, check=False, **kw):
    ...
    try:
        ...
        print(
            "[%s] An exception has occurred with command: '%s'"
            % (what, (" ").join(cmd))
        )
        print(e)
        sys.exit(1)
...
def start_CAN_Periph(opts, frame_info):
    ...
    defaults_path = ",".join(
        [util.relcurdir(os.path.join(autotest_dir, p)) for p in defaults_path]
    )
...
def start_vehicle(binary, opts, stuff, spawns=None):
    ...
    if opts.add_param_file:
        for file in opts.add_param_file:
            if not os.path.isfile(file):
                print("The parameter file (%s) does not exist" % (file,))
                sys.exit(1)
    ...
    if cmd_opts.start_time is not None:
        ...
        try:
            start_time_UTC = time.mktime(
                datetime.datetime.strptime(
                    cmd_opts.start_time, "%Y-%m-%d-%H:%M"
                ).timetuple()
            )
        except Exception:
            print(
                "Incorrect start time format - require YYYY-MM-DD-HH:MM (given %s)"
                % cmd_opts.start_time
            )
            sys.exit(1)
...
group_build.add_option(
    "", "--coverage", action="store_true", default=False, help="use coverage build"
)
group_build.add_option(
    "",
    "--ubsan",
...
group_sim.add_option(
    "-I", "--instance", default=0, type="int", help="instance of simulator"
)
group_sim.add_option(
    "-n",
    "--count",
...
group_sim.add_option(
    "", "--udp", action="store_true", default=False, help="Use UDP on 127.0.0.1:5760"
)
group_sim.add_option(
...
group_sim.add_option(
    "--flash-storage", action="store_true", help="use flash storage emulation"
)
group_sim.add_option(
    "--fram-storage", action="store_true", help="use fram storage emulation"
)
group_sim.add_option("--enable-ekf2", action="store_true", help="disable EKF2 in build")
group_sim.add_option(
    "--disable-ekf3", action="store_true", help="disable EKF3 in build"
)
group_sim.add_option(
    "",
    "--start-time",
...
group_sim.add_option(
    "--postype-single", action="store_true", help="force single precision postype_t"
)
group_sim.add_option(
    "--ekf-double", action="store_true", help="use double precision in EKF"
)
group_sim.add_option(
    "--ekf-single", action="store_true", help="use single precision in EKF"
)
group_sim.add_option(
    "", "--slave", type="int", default=0, help="Set the number of JSON slave"
)
group_sim.add_option(
    "",
    "--auto-sysid",
...
group_sim.add_option(
    "--disable-networking", action="store_true", help="Disable networking APIs"
)
group_sim.add_option("--enable-ppp", action="store_true", help="Enable PPP networking")
group_sim.add_option(
    "--enable-networking-tests", action="store_true", help="Enable networking tests"
)
...
group.add_option(
    "", "--map", default=False, action="store_true", help="load map module on startup"
)
group.add_option(
    "",
    "--console",
...
group.add_option(
    "", "--aircraft", default=None, help="store state and logs in named directory"
)
...
group_completion.add_option(
    "", "--list-vehicle", action="store_true", help="List the vehicles"
)
group_completion.add_option(
    "", "--list-frame", type="string", default=None, help="List the vehicle frames"
)
...
if (
    cmd_opts.can_peripherals
    or frame_infos.get("periph_params_filename", None) is not None
):
...
if cmd_opts.swarm is not None:
    ...
    for x in instances:
        offsets[x] = [
            dist * math.sin(math.radians(bearing)),
            dist * math.cos(math.radians(bearing)),
            0,
            0,
        ]
        dist += metres
...
if True:
    ...
    if cmd_opts.vehicle_binary is not None:
        ...
        binary_basedir = "build/sitl"
        vehicle_binary = os.path.join(
            root_dir, binary_basedir, frame_infos["waf_target"]
        )
