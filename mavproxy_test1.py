...
class example1(mp_module.MPModule):
    def __init__(self, mpstate):
        """Initialise module"""
        super(example1, self).__init__(mpstate, "example1", "")
        ...
        self.example1_settings = mp_settings.MPSettings(
            [ ('verbose', bool, False),
          ])
        self.add_command('example1', self.cmd_example1, "example1 module", ['status','set (LOGSETTING)'])

    def usage(self):
        '''show help on command line options'''
        return "Usage: example <status|set>"

    def cmd_example1(self, args):
        ...
        if len(args) == 0:
            ...
            self.example1_settings.command(args[1:])
        else:
            print(self.usage())
    ...
    def boredom_message(self):
        if self.example1_settings.verbose:
