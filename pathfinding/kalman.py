...
class UAVKalmanFilter:
    def __init__(self, target_dub_pos:DubinPoint, measurement_noise_variance, process_noise_variance):
        ...
        self.kf = KalmanFilter(dim_x=3, dim_z=3) # Updated to include theta in the measurements
        
        # Measurement matrix (H)
        self.kf.H = np.array([[1, 0, 0],
                              [0, 1, 0],
                              [0, 0, 1]]) # Updated to include theta
        ...
        self.kf.R = np.array([[measurement_noise_variance, 0, 0],
                              [0, measurement_noise_variance, 0],
                              [0, 0, measurement_noise_variance]]) # Updated to include theta
    ...
    def update_state_transition_matrix(self, dt, velocity):
        ...
        self.kf.F = np.array([[1, 0, -dt * velocity],
                              [0, 1, 0],
                              [0, 0, 1]])
        
        # Process noise covariance (Q)
        # Assuming no control input affects the state directly
        self.kf.Q = Q_discrete_white_noise(dim=3, dt=dt, var=self.process_noise_variance) ## not sure if it is correct
...
if __name__ == '__main__':
    ...
    uav_kalman_filter = UAVKalmanFilter(DubinPoint(0,0,0),measurement_noise_variance, process_noise_variance)
