...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d0 = paddle.nn.Conv2D(stride=2, padding=2, out_channels=16, kernel_size=(6, 6), in_channels=3)
        ...
        self.conv2d7 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=64, kernel_size=(3, 3), in_channels=32)
        ...
        self.conv2d12 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        ...
        self.conv2d15 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=128, kernel_size=(3, 3), in_channels=64)
        ...
        self.conv2d20 = paddle.nn.Conv2D(padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        ...
        self.conv2d25 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=256, kernel_size=(3, 3), in_channels=128)
        ...
        self.conv2d42 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        ...
        self.conv2d45 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        ...
        self.conv2d51 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128)
        ...
        self.conv2d54 = paddle.nn.Conv2D(padding=1, out_channels=128, kernel_size=(3, 3), in_channels=128)
        ...
        self.conv2d60 = paddle.nn.Conv2D(padding=1, out_channels=80, kernel_size=(3, 3), in_channels=64)
        ...
        self.conv2d66 = paddle.nn.Conv2D(padding=1, out_channels=80, kernel_size=(3, 3), in_channels=128)
        self.silu63 = paddle.nn.Silu()
        self.conv2d67 = paddle.nn.Conv2D(padding=1, out_channels=80, kernel_size=(3, 3), in_channels=80)
        self.silu64 = paddle.nn.Silu()
        self.conv2d68 = paddle.nn.Conv2D(out_channels=80, kernel_size=(1, 1), in_channels=80)
        ...
        self.conv2d71 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.conv2d72 = paddle.nn.Conv2D(padding=1, out_channels=80, kernel_size=(3, 3), in_channels=256)
        self.silu67 = paddle.nn.Silu()
        self.conv2d73 = paddle.nn.Conv2D(padding=1, out_channels=80, kernel_size=(3, 3), in_channels=80)
        self.silu68 = paddle.nn.Silu()
        self.conv2d74 = paddle.nn.Conv2D(out_channels=80, kernel_size=(1, 1), in_channels=80)
        ...
        self.x950 = self.create_parameter(dtype='float32', shape=(1, 2, 8400), default_initializer=paddle.nn.initializer.Constant(value=0.0))
    ...
    def forward(self, x0):
        ...
        x734 = 2
        x735 = 1
        x761 = self.conv2d57(x586)
        x762 = self.silu57(x761)
        x770 = self.conv2d58(x762)
        x771 = self.silu58(x770)
        x778 = self.conv2d59(x771)
        x789 = self.conv2d60(x586)
        x790 = self.silu59(x789)
        x798 = self.conv2d61(x790)
        x799 = self.silu60(x798)
        x806 = self.conv2d62(x799)
        x807 = [x778, x806]
        x808 = paddle.concat(x=x807, axis=1)
        x819 = self.conv2d63(x658)
        x820 = self.silu61(x819)
        x828 = self.conv2d64(x820)
        x829 = self.silu62(x828)
        x836 = self.conv2d65(x829)
        x847 = self.conv2d66(x658)
        x848 = self.silu63(x847)
        x856 = self.conv2d67(x848)
        x857 = self.silu64(x856)
        x864 = self.conv2d68(x857)
        x865 = [x836, x864]
        x866 = paddle.concat(x=x865, axis=1)
        x877 = self.conv2d69(x730)
        x878 = self.silu65(x877)
        x886 = self.conv2d70(x878)
        x887 = self.silu66(x886)
        x894 = self.conv2d71(x887)
        x905 = self.conv2d72(x730)
        x906 = self.silu67(x905)
        x914 = self.conv2d73(x906)
        x915 = self.silu68(x914)
        x922 = self.conv2d74(x915)
        x923 = [x894, x922]
        x924 = paddle.concat(x=x923, axis=1)
        x926 = paddle.reshape(x=x808, shape=[1, 144, -1])
        x928 = paddle.reshape(x=x866, shape=[1, 144, -1])
        x930 = paddle.reshape(x=x924, shape=[1, 144, -1])
        x931 = [x926, x928, x930]
        x932 = paddle.concat(x=x931, axis=2)
        x934 = paddle.split(x=x932, num_or_sections=[64, 80], axis=1)
        x935, x936 = x934
        x939 = paddle.reshape(x=x935, shape=[1, 4, 16, 8400])
        x940_shape = x939.shape
        x940_len = len(x940_shape)
        x940_list = []
        for i in range(x940_len):
            x940_list.append(i)
        if x734 < 0:
            x734_new = x734 + x940_len
        else:
            x734_new = x734
        if x735 < 0:
            x735_new = x735 + x940_len
        else:
            x735_new = x735
        x940_list[x734_new] = x735_new
        x940_list[x735_new] = x734_new
        x940 = paddle.transpose(x=x939, perm=x940_list)
        x941 = self.softmax0(x940)
        x947 = self.conv2d75(x941)
        x949 = paddle.reshape(x=x947, shape=[1, 4, 8400])
        x950 = self.x950
        x951 = paddle.split(x=x949, num_or_sections=2, axis=1)
        x952, x953 = x951
        x954 = x950 - x952
        x955 = x950 + x953
        x956 = x954 + x955
        ...
        x958 = x955 - x954
        x959 = [x957, x958]
        x960 = paddle.concat(x=x959, axis=1)
        x961 = x960 * x731
        x962 = self.sigmoid0(x936)
        x963 = [x961, x962]
        x964 = paddle.concat(x=x963, axis=1)
        return x964
...
def main(x0):
    ...
    params = paddle.load(r'C:\Users\ihsan\npmbugsolve\UAV_teknofest\yolov5nu_paddle_model\model.pdparams')
